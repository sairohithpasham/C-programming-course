...
int is_n_length_straight_at(deck_t * hand, size_t index, suit_t fs, int n)
{
  ...
  if(fs == NUM_SUITS)
	...
	return 0;
      card_t * curr = hand->cards[index]; 
      for(int i = index+1; i < hand->n_cards; i++)
	{
	  if(hand->cards[i]->suit != fs)
	    continue;
	  if(curr->value-1 == hand->cards[i]->value)
	    {
	      count++;
	      if(count==n)
		return 1;
...
int check_idx_in_arr(deck_t * hand, int idx, hand_eval_t * hand_eval)
{
  for(int i=0; i<5; i++)
    {
...
card_t * helper_for_build(hand_eval_t * hand_eval, deck_t * hand)
{
  for(int i =0; i<hand->n_cards;i++)
    {
      if(!check_idx_in_arr(hand, i, hand_eval))
	 return hand->cards[i];
    }
      exit(EXIT_FAILURE);
}
...
hand_eval_t build_hand_from_match(deck_t * hand,
  ...
  for(int i=0;i<5-n;i++)
    {
      ans.cards[n+i] = helper_for_build(&ans, hand);
    }
  return ans;
}
...
int compare_hands(deck_t * hand1, deck_t * hand2) {
  ...
  hand_eval_t hand_eval_1 = evaluate_hand(hand1);
  hand_eval_t hand_eval_2 = evaluate_hand(hand2);

  if(hand_eval_1.ranking != hand_eval_2.ranking)
    {
      return hand_eval_2.ranking - hand_eval_1.ranking;
    }
  for(int i = 0; i < 5; i++)
    {
...
unsigned * get_match_counts(deck_t * hand)
{
  ...
  for(int i=0;i<hand->n_cards;i++)
    {
      counts[i] = helper(hand,*hand->cards[i]);
    }
  return counts;
}
